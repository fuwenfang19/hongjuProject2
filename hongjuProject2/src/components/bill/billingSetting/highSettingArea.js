...
export default class billingSlide extends React.Component {
    ...
    handleChangeFirstSelect = (value) => {
        const newhightBillSettingData = this.props.hightSettingData.concat()
    ...
    changeSettinghighCheckbox = (e, i) => {
        const newHighSettingData = this.props.hightSettingData.concat()
        ...
        const status = newHighSettingData[i].isChosed
        //如果选中 并且还没有选择配置参数的时候  则给一个提示
        // if (!status && !isChangeFirtParams) {
        //     Toast.warning('请先设置字段参数，再点击选择哟！');
        // }
        newHighSettingData[i].isChosed = !status
    ...
    handleHighSeletAll = (e) => {
        ...
        if (!selectHighAll) {
            ...
            //console.log(this.props.hightSettingData)
    ...
    render() {
        ...
        const RelationTypes = this.props.highSettingEnums.RelationTypes
        let newhightBillSettingData = this.props.hightSettingData.concat()
        const fakeSelfDefineFileData = this.props.highSettingEnums.fakeSelfDefineData
        // const fakeSelfDefineExpandData = this.props.highSettingEnums.fakeSelfDefineExpandData
      
        //console.log(fakeSelfDefineFileData.length)
        ...
        newhightBillSettingData.map((item, index) => {
                            ...
                            >
                                {
                                    item.attrs.relation_type === 1 ?
                                        fields_template_1_useful.map((item, i) => {
                                            return (
                                                <Option value={item.defid + '&' + index} key={i}>{item.name}</Option>
                                            )
                        ...
                        <div style={{width:200}}>
                            {
                                fakeSelfDefineFileData.length === 0 ? (
                                    ...
                                    <div>
                                        <Select style={{ minWidth: 77,width:60,margin:0}}
                                                ...
                                                value={item.attrs.custom_objectid === undefined && item.attrs.relation_col === undefined? fakeSelfDefineFileData[0].id + '&' + fakeSelfDefineFileData[0].field_data.defid + '&' + index:item.attrs.custom_objectid + '&' + item.attrs.relation_col + '&' + index}>
                                            {
                                                fakeSelfDefineFileData.map((item, i) => {
                                                    return (
                                                        <Option
                                                            value={item.id + '&' + item.field_data.defid + '&' + index}
                                                            title={item.name}
                                      ...
                                      { /* 
                                        fakeSelfDefineExpandData.length === 0 ? ''
                                            :
                                            <Select
                                                mode="multiple"
                                                placeholder="请选择"
                                                size={'small'}
                                                defaultValue={arr}
                                                style={{ width: '47%' ,margin:0,paddingLeft:4}}
                                            >
                                                {fakeSelfDefineExpandData.map((item, i) => {
                                                    return <Option
                                                        value={item['name'].toString() + '&' + item['value']}
        ...
        return (
            <div className='highBillingSettingArea'>
                ...
                <div className='billingHighConWrap' style={{ height: clientHeight - 170 }}>
